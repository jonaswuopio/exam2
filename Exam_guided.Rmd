...
read_in <- getwd() #My parent directory is set to my exam folder

# List all the files from the folder structure.
all_files <- list.files(path = read_in, recursive = FALSE, pattern = ".txt", full.names = FALSE)
all_files
...
all_files_name <- paste0(
  "A_",  # Prefix "A_"
  ifelse(seq_along(all_files) %% 2 == 1, "BID", "R2"),  # Alternate between "R2" and "BID"
  "_",  
  gsub(".txt", "", basename(all_files))  # Remove ".txt" 
)

####I have no idea what you mean here but the code above provides files name according to the helpfile / Jonas

#all_files_name <- paste0(gsub(all_files, "([A-Z]_)?R2|______"), 
                        ### '_', 
...
#I did not get your suggestion. Also column Angle has "," as decimal some places which makes it class character and not dbl. The code provided below seems to work according to instructions and helpfile with correct class. Also, data starts on row 27, why I skip the first 26.  
...
#####I skip this because I already did this in output 2.
...
df_lst_long <- df_lst %>%
 map(~ mutate(.x, time = row_number()))  # use you index to create a new column time
  
df_lst_long <- df_lst_long %>% map(~ pivot_longer(.x, cols = -time)) # convert to long format 
...
coord_min_list <- coord_min_list %>% 
  ...
  # Apply pivot_wider after ensuring grouping by 'segment'
...
It seems as the code finds the min torque value and the max torque value of the prior list created (coord_min_list and coord_max_list) and put them in two lists (min_list and max_list)
...
```{r}
setwd("C:/Users/jonwuo/OneDrive - Karolinska Institutet/Dokument/Forskning/Kurser/Intermediate R/Exam")

speed <- all_files %>%  # Use all_files (list of file paths) as input
  map(~ read.table(.x, nrows = 1, header = TRUE, col.names = 'info')) %>%  # Read the first row of each file and assign column name 'info'
  map(. %>% tibble::rowid_to_column(var = 'id')) %>%  # Add row IDs as a column called 'id'
  map(. %>% unite('note', id:info, sep = ':speed', remove = T)) %>%  # Unite 'id' and 'info' into one column 'note'
  set_names(all_files)  # Set names of the resulting list based on the file names
...
plots_all <- lapply(1:length(df_lst_long), function(i) {
  ...
  ggplot(data = df_lst_long[[i]], aes(x = time, y = value)) + # define main ggplot 
    ...
    theme(
          ...
          panel.grid.major.x = element_blank(),# remove grid.minor on both axis
          panel.grid.major.y = element_line(color = "grey"), # remove grid.major only on the x-axis
